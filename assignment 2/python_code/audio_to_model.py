...
def my_mfcc(file_path):
    x, sr = read_audio(file_path)
    ...
    mel_data, energy = mel_filter(fft_data,sr)  # mel filters(outputs log mel-filter)
    ...
    mfcc_with_delta2 = np.insert(mfcc_with_delta1, len(mfcc_with_delta1[0]), values=delta_square,
                                 axis=1)  # append second delta
