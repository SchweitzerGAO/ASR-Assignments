...
def EM_HMM_training(training_file='./csvfiles/training_csv', DIM=20, num_model=10, num_state=13):
    """
    :param training_file:
    :param DIM:
    :param num_model:
    ...
    num_iter = 25  # bigger than 10
    log_likelihood_iter = np.full((1, num_iter), 0)
    likelihood_iter = np.full((1, num_iter), 0)
                ...
                with open(file, 'rb') as data:
                    ...
                    for feature in dic.values():
                        mean_numerator, var_numerator, aij_numerator, denominator, log_likelihood_i, likelihood_i = \
                            EM_HMM(hmm.mean[label, :, :], hmm.var[label, :, :], hmm.aij[label, :, :], feature)

                        sum_mean_numerator[label, :, :] += mean_numerator[:, 1:-1]
                        sum_var_numerator[label, :, :] += var_numerator[:, 1:-1]
                        sum_aij_numerator[label, :, :] += aij_numerator[1:-1, 1:-1]
                        sum_denominator[:, label] += denominator[1:-1]
    ...
    with open('./HMM_model/hmm.pkl', 'a') as f:
        pkl.dump(hmm, f)
...
def initial_HMM(hmm: HMM, num_of_state, num_of_model, sum_of_features, sum_of_features_square,
    ...
    for k in range(0, num_of_model):
        for m in range(0, num_of_state):
            hmm.mean[k, :, m] = sum_of_features / num_of_feature
            hmm.var[k, :, m] = sum_of_features_square / num_of_feature - hmm.mean[k, :, m] * hmm.mean[k, :, m]
    ...
    with open(training_file) as f:
        ...
        for row in reader:
            ...
            with open(file, 'rb') as data:
                dic = pkl.load(data)
                for feature in dic.values():
                    sum_of_feature += np.sum(feature, 1)
                    sum_of_feature_square += np.sum(feature ** 2, 1)
